...
def run_analysis():
    ...
    try:
        ...
        if not df.empty:
            ...
            # 使用 .copy() 確保修改欄位時不觸發 SettingWithCopyWarning 警告
            df = df.copy()
            
            # 確保精確採用還原收盤價 (Adj Close)
    ...
    # 總經與籌碼指標開關 (手動調整區：True 代表觸發，False 代表安全)
    signals = {
        "US_Yield_Inversion": True, 
        "TW_Foreign_Short": True, 
        "TW_Margin_Overheat": False,
        "US_Speculative_Crash": True, 
        "US_Valuation_Bubble": True, 
        "TW_Tech_Divergence": True
    }
