...
def run_analysis():
    ...
    # 初始化預設變數 (防止報錯時無變數可用)
    current_price, ma20, high_water_mark, drawdown_pct = 0, 0, 0, 0
    fetch_success = False
    ...
    trend_broken = current_price < ma20 if fetch_success else False
    damage_taken = drawdown_pct >= 7.0 if fetch_success else False

    # 3. 決策邏輯 (加入資料失效的保護狀態)
    if not fetch_success:
        decision, bg_color = "⚠️ 股價 API 連線異常，本次僅計算環境風險，請稍後再試。", "#A9A9A9" # 灰色
